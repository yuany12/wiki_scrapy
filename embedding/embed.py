...
class author_embedding:
    ...
    def build_graph(self):
        cur = self.conn_db('arnet_db').cursor()
        cur.execute("select id from publication")
        self.vertices = collections.defaultdict(list)
        self.vocab = []
        cnt, tot = 0, cur.rowcount
        for row in cur.fetchall():
    ...
    def save_graph(self):
        cPickle.dump(self.vertices, open('authors.graph.dump', 'wb'))
        cPickle.dump(self.vocab, open('author_vocab.graph.dump', 'wb'))

    def load_graph(self):
        self.vertices = cPickle.load(open('authors.graph.dump', 'rb'))
        self.vocab = cPickle.load(open('author_vocab.graph.dump', 'rb'))
